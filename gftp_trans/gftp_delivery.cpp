                                 ...
                                 size_t datasize_inB, void* data_)
{
  ...
  if (gftp_driver_->put_file(src_url, dst_url) ) {
    LOG(ERROR) << "put_over_gftp:: gftp_driver_->init_file_transfer failed!";
    return 1;
  }
  // 
...
size_t GFTPDDManager::get_over_gftp(std::string s_laddr, std::string s_lport, std::string s_tmpfs_dir,
                                    std::string key, unsigned int ver, 
                                    size_t &datasize_inB, void* data_) //returns datasize_inB
{
  ...
  if (gftp_driver_->get_file(src_url, dst_url) ) {
    LOG(ERROR) << "get_over_gftp:: gftp_driver_->init_file_transfer failed!";
    return 1;
  }
