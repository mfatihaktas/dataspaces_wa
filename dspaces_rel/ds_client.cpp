...
std::map<std::string, std::string> IMsgCoder::decode(char* msg)
{
  ...
  ia >> msg_map;
  // 
  // LOG(INFO) << "decode:: msg_map= \n" << patch_ds::str_str_map_to_str(msg_map);
  return msg_map;
}
...
//************************************   RQTable   ********************************//
RQTable::RQTable() { LOG(INFO) << "RQTable:: constructed."; }

RQTable::~RQTable() { LOG(INFO) << "RQTable:: deconstructed."; }
...
std::map<std::string, std::string> RQTable::to_unmarked_str_str_map()
{
  ...
  for (std::map<key_ver_pair, char>::iterator it = key_ver__dsid_map.begin(); it != key_ver__dsid_map.end(); it++) {
    ...
    if (!key_ver__mark[kv]) {
      ...
      str_str_map["lb_" + counter_str] = lb;
      str_str_map["ub_" + counter_str] = ub;
      str_str_map["data_type_" + counter_str] = key_ver__data_type_map[kv];
...
: app_id(app_id),
  ...
  ds_driver_( new DSpacesDriver(app_id, num_cnodes) ),
  ...
  pbuffer_(new PBuffer(with_prefetch, buffer_size, boost::bind(&RIManager::handle_prefetch, this, _1), 
...
int RIManager::remote_get(char ds_id, std::map<std::string, std::string> r_get_map)
{
  ...
  key_ver_pair kv = std::make_pair(key, ver);
  remote_get_time_profiler.add_event(kv, ("<" + key + ", " + boost::lexical_cast<std::string>(ver) + ">") );
  ...
  remote_get_time_profiler.end_event(kv);
  LOG(INFO) << "remote_get:: remote_get_time_profiler= \n" << remote_get_time_profiler.to_str();
  // 
  patch_ds::free_all<uint64_t*>(3, gdim_, lb_, ub_);
...
void RIManager::handle_prefetch(std::map<std::string, std::string> prefetch_map)
{
  ...
  if (ds_id == this->id) {
    ...
    // TODO: <key, ver> may be being fetched, should check for it. This can happen when 
    // there is no gap between remote gets done by the user app.
    key_ver_pair kv = std::make_pair(key, ver);
    if (key_ver_being_fetched_vector.contains(kv) ) {
      LOG(INFO) << "handle_get:: <key= " << key << ", ver= " << ver << "> is being fetched right now...";
      being_fetched_syncer.add_sync_point(kv, 1);
      ...
      if (imsg_coder.encode_msg_map(r_g_map, key, ver, data_type, size, ndim, gdim_, lb_, ub_) ) {
        LOG(ERROR) << "handle_prefetch:: imsg_coder.encode_msg_map failed! for <key= " << key << ", ver= " << ver << ">.";
      }
...
{
  LOG(INFO) << "handle_r_query:: r_query_map= \n" << patch_ds::str_str_map_to_str(r_query_map);
...
{
  LOG(INFO) << "handle_rq_reply:: rq_reply_map= \n" << patch_ds::str_str_map_to_str(rq_reply_map);
...
{
  LOG(INFO) << "handle_r_fetch:: r_fetch_map= \n" << patch_ds::str_str_map_to_str(r_fetch_map);
...
{
  LOG(INFO) << "handle_r_rqtable:: r_rqtable_map= \n" << patch_ds::str_str_map_to_str(r_rqtable_map);
  ...
  while(1) {
    ...
    pbuffer_->reg_key_ver__pkey_pver_pair(key, ver);
...
{
  LOG(INFO) << "handle_r_place:: r_place_map= \n" << patch_ds::str_str_map_to_str(r_place_map);
...
{
  LOG(INFO) << "handle_rp_reply:: rp_reply_map= \n" << patch_ds::str_str_map_to_str(rp_reply_map);
...
{
  LOG(INFO) << "handle_r_subscribe:: r_subs_map= \n" << patch_ds::str_str_map_to_str(r_subs_map);
...
{
  LOG(INFO) << "handle_gftpb_ping:: gftpb_ping_map= \n" << patch_ds::str_str_map_to_str(gftpb_ping_map);
...
{
  LOG(INFO) << "handle_gftpb_pong:: gftpb_pong_map= \n" << patch_ds::str_str_map_to_str(gftpb_pong_map);
