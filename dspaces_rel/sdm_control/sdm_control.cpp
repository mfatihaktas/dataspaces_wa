...
int SDMSlave::get(int app_id, bool blocking, std::string key, unsigned int ver, COOR_T* lcoor_, COOR_T* ucoor_)
{
  ...
  bool did_send_sdm_squery = false;
  ...
  if (qtable_->query(key, ver, lcoor_, ucoor_, p_id_v) ) {
    ...
    sdm_s_syncer.wait(sync_point);
    sdm_s_syncer.del_sync_point(sync_point);
    log_(INFO, "done waiting on sdm_squery_reply; " << KV_LUCOOR_TO_STR(key, ver, lcoor_, ucoor_) )
    ...
    if (qtable_->query(key, ver, lcoor_, ucoor_, p_id_v) ) {
      ...
      log_s(sdm_log_f, INFO, "Still MISS; " << KV_LUCOOR_TO_STR(key, ver, lcoor_, ucoor_)
                             << "after sdm_squery")
      return 1;
    }
  }
...
int SDMSlave::notify_remote_get_done(std::string key, unsigned int ver, COOR_T* lcoor_, COOR_T* ucoor_)
{
  int err;
  std::map<std::string, std::string> msg_map;
  return_if_err(msg_coder.encode_msg_map(msg_map, NDIM, key, ver, lcoor_, ucoor_), err)
  
  msg_map["type"] = SDM_MOVE_REPLY;
  return_if_err(send_cmsg_to_master(msg_map), err)
  return 0;
}
...
void SDMSlave::handle_sdm_squery_reply(std::map<std::string, std::string> msg_map)
{
  ...
  sdm_s_syncer.notify(patch_sdm::hash_str(
    SDM_SQUERY + "_" + patch_sdm::get_data_id(data_id_t, msg_map) ) );
}
...
int SDMMaster::sdm_mquery(bool blocking, int c_id, std::string key, unsigned int ver, COOR_T* lcoor_, COOR_T* ucoor_)
{
  int err;
  std::string data_id = patch_sdm::get_data_id(data_id_t, key, ver, lcoor_, ucoor_);
  // Note: Some get requests may get through wait_for_get in RIManager::handle_get so will check here to avoid double remote_put/get
  // std::vector<std::string>& moving_data_id_v = ds_id__moving_data_id_v_map[get_id() ];
  // if (std::find(moving_data_id_v.begin(), moving_data_id_v.end(), data_id) != moving_data_id_v.end() ) {
  //   log_(WARNING, "already moving the data here; data_id= " << data_id)
  //   return 0;
  // }
  
  // Note: unnecessary as long as the master is notified by the peers about any put
  // std::map<std::string, std::string> msg_map;
  ...
  // unsigned int sync_point = patch_sdm::hash_str(SDM_MQUERY + "_" + data_id);
  ...
  if (wa_space_->query(key, ver, lcoor_, ucoor_, ds_id_v) ) {
    ...
    if (blocking) {
      ...
      unsigned int sync_point = patch_sdm::hash_str("B" + SDM_QUERY + "_" + data_id);
  ...
  if (move_act) {
    ...
    return_if_err(msg_coder.encode_msg_map(move_act_msg_map, NDIM, key, ver, lcoor_, ucoor_), err)
    move_act_msg_map["type"] = SDM_MOVE;
    move_act_msg_map["id"] = boost::lexical_cast<std::string>(from_id);
    move_act_msg_map["to_id"] = boost::lexical_cast<std::string>(get_id() );
    return_if_err(send_cmsg(from_id, move_act_msg_map), err)
    ...
    log_(INFO, "waiting move; from_id= " << from_id << " to here, data_id= " << data_id)
    unsigned int sync_point = patch_sdm::hash_str(SDM_MOVE + "_" + data_id);
    sdm_m_syncer.add_sync_point(sync_point, 1);
    sdm_m_syncer.wait(sync_point);
    sdm_m_syncer.del_sync_point(sync_point);
    log_(INFO, "done waiting move; from_id= " << from_id << "to here, data_id= " << data_id)
  }
...
int SDMMaster::put(bool notify, std::string key, unsigned int ver, COOR_T* lcoor_, COOR_T* ucoor_, int p_id)
{
  ...
  unsigned int sync_point = patch_sdm::hash_str(
    "B" + SDM_QUERY + "_" + patch_sdm::get_data_id(data_id_t, key, ver, lcoor_, ucoor_) );
...
int SDMMaster::get(int c_id, bool blocking, std::string key, unsigned int ver, COOR_T* lcoor_, COOR_T* ucoor_)
{
  ...
  if (qtable_->query(key, ver, lcoor_, ucoor_, p_id_v) ) {
    ...
    log_s(sdm_log_f, INFO, "MISS; " << KV_LUCOOR_TO_STR(key, ver, lcoor_, ucoor_) << "\n"
                           << "\t blocking= " << blocking)
    if (blocking) {
      if (sdm_mquery(blocking, c_id, key, ver, lcoor_, ucoor_) ) {
...
int SDMMaster::notify_remote_get_done(std::string key, unsigned int ver, COOR_T* lcoor_, COOR_T* ucoor_)
{
...
{
  std::string data_id = patch_sdm::get_data_id(data_id_t, msg_map);
  log_(INFO, "waiting move; from_id= " << from_id << ", to_id= " << to_id << ", data_id= " << data_id)
  ...
  sdm_m_syncer.wait(sync_point);
  sdm_m_syncer.del_sync_point(sync_point);
  log_(INFO, "done waiting move; from_id= " << from_id << ", to_id= " << to_id << ", data_id= " << data_id)
