 ...
 */
void
mfa_hilbert_i2c(unsigned nDims, unsigned nBits, bitmask_t index, bitmask_t coord[])
{
  if (nDims > 1)
    {
      bitmask_t coords;
 ...
 */
bitmask_t
mfa_hilbert_c2i(unsigned nDims, unsigned nBits, bitmask_t const coord[])
{
  if (nDims > 1)
    {
      unsigned const nDimsBits = nDims*nBits;
...
/*****************************************************************
 * mfa_hilbert_cmp, mfa_hilbert_ieee_cmp
 * 
 * Determine which of two points lies further along the Hilbert curve
 * Inputs:
 *  nDims:      Number of coordinates.
 *  nBytes:     Number of bytes of storage/coordinate (mfa_hilbert_cmp only)
 *  nBits:      Number of bits/coordinate. (mfa_hilbert_cmp only)
...
static int
mfa_hilbert_cmp_work(unsigned nDims, unsigned nBytes, unsigned nBits,
...
int
mfa_hilbert_cmp(unsigned nDims, unsigned nBytes, unsigned nBits,
      void const* c1, void const* c2)
{
  bitmask_t const one = 1;
  bitmask_t bits = one << (nDims-1);
  return mfa_hilbert_cmp_work(nDims, nBytes, nBits, 0, nBits,
        (char const*)c1, (char const*)c2,
        0, bits, bits, getIntBits);
}

int
mfa_hilbert_ieee_cmp(unsigned nDims, double const* c1, double const* c2)
{
  ...
  return mfa_hilbert_cmp_work(nDims, 8, 64, max, max+53,
        (char const*)c1, (char const*)c2,
...
static unsigned
mfa_hilbert_box_vtx_work(unsigned nDims, unsigned nBytes, unsigned nBits,
         int findMin,
         unsigned max, unsigned y,
         char* c1, char* c2,
         unsigned rotation,
...
unsigned
mfa_hilbert_box_vtx(unsigned nDims, unsigned nBytes, unsigned nBits,
    int findMin, void* c1, void* c2)
{
  bitmask_t const one = 1;
  bitmask_t bits = one << (nDims-1);
  return mfa_hilbert_box_vtx_work(nDims, nBytes, nBits, findMin,
            0, nBits, (char*)c1, (char*)c2,
            0, bits, bits, getIntBits);
}

unsigned
mfa_hilbert_ieee_box_vtx(unsigned nDims,
         int findMin, double* c1, double* c2)
{
  ...
  return mfa_hilbert_box_vtx_work(nDims, 8, 64, findMin,
 ...
 */
unsigned
mfa_hilbert_box_pt_work(unsigned nDims, unsigned nBytes, unsigned nBits,
        int findMin,
        unsigned max, unsigned y,
        char* c1, char* c2,
        unsigned rotation,
...
unsigned
mfa_hilbert_box_pt(unsigned nDims, unsigned nBytes, unsigned nBits,
    int findMin, void* c1, void* c2)
{
  bitmask_t const one = 1;
  bitmask_t bits = one << (nDims-1);
  return mfa_hilbert_box_pt_work(nDims, nBytes, nBits, findMin,
           0, nBits, (char*)c1, (char*)c2,
           0, bits, bits,
...
unsigned
mfa_hilbert_ieee_box_pt(unsigned nDims,
        int findMin, double* c1, double* c2)
{
  unsigned rotation, max;
  bitmask_t bits, index;
  ...
  if (c1Signs != c2Signs)
    {
      ...
      index = 1;
      mfa_hilbert_box_pt_work(nDims, 8, 64, findMin,
  ...
  return mfa_hilbert_box_pt_work(nDims, 8, 64, findMin,
           max, max+53, (char *)c1, (char *)c2,
           rotation, bits, index,
           getIEEEBits, propogateIEEEBits);
}
 ...
 */
int
mfa_hilbert_nextinbox(unsigned nDims, unsigned nBytes, unsigned nBits,
     ...
     dimensions don't matter. */
  {
        ...
        for (d = 0; d < nDims; ++d)
    {
      ...
      cc2[byteId] = hipart | (lo2 &~ hibits);
        ...
        mfa_hilbert_box_pt(nDims, nBytes, nBits, !findPrev, c1V, c2V);
        return 1;
      }
  }
...
void
mfa_hilbert_incr(unsigned nDims, unsigned nBits, bitmask_t coord[])
{
  bitmask_t const one = 1;
  bitmask_t const ndOnes = ones(bitmask_t,nDims);
  bitmask_t const nd1Ones= ndOnes >> 1;
...
/* Revision history:
   ...
   Dec 1998: Fixed bug in mfa_hilbert_c2i that allowed a shift by number of bits in
   bitmask to vaporize index, in last bit of the function.  Implemented
   mfa_hilbert_incr.

   August 1999: Added argument to mfa_hilbert_nextinbox so that you can, optionally,
   ...
   loops in mfa_hilbert_cmp, with the intention of reusing the cmp code to compare
...
#define maxDim (8*sizeof(bitmask_t))
  ...
  for (;;)
    ...
    int miscount = 0;
          mfa_hilbert_i2c( nDims, nBits, r, coord );
          printf("%d: ", (unsigned)r);
          for (i = 0; i < nDims; ++i)
      {
    ...
    if (r > 0 && miscount != 1)
      printf(".....error");
    printf("\n");
          r1 = mfa_hilbert_c2i( nDims, nBits, coord );
          if ( r != r1 )
    ...
    for (r1 = 0; r1 < r; ++r1 )
      {
        unsigned ans;
        mfa_hilbert_i2c( nDims, nBits, r1, coord1 );
        ans = mfa_hilbert_cmp( nDims, sizeof(coord[0]), nBits, coord, coord1);
        if (ans != 1)
    {
    ...
    mfa_hilbert_i2c( nDims, nBits, r1, coord1 );
    if (mfa_hilbert_cmp( nDims, sizeof(coord[0]), nBits, coord, coord1) != 0)
...
int cmp(const void* xv, const void* yv)
{
  double const* x = xv;
  double const* y = yv;
  /* return mfa_hilbert_cmp(2, 8, 64, x, y); */
  return mfa_hilbert_ieee_cmp(2, x, y);
}
...
int main()
{
  ...
  coord[i] = ((k>>i)&1)? -1.: 1.;
      ...
      mfa_hilbert_ieee_cmp( nDims, coord, coord);
    }
  return 0;
}
...
{
  return mfa_hilbert_cmp(g_nDims, sizeof(unsigned), 8*sizeof(unsigned), c1p, c2p);
}
...
int main()
{
  ...
  mfa_hilbert_box_vtx(nDims, sizeof(unsigned), 8*sizeof(unsigned),
      1, cornerlo, work);
...
int cmp(void const* c1p, void const* c2p)
{
  return mfa_hilbert_ieee_cmp(g_nDims, c1p, c2p);
}
...
int main()
{
  ...
  mfa_hilbert_box_pt(nDims, sizeof(unsigned), 8*sizeof(unsigned),
     1, pointlo, work);
...
int main()
{
  ...
  mfa_hilbert_ieee_box_pt(nDims, 0, work, pointhi);
  printf("Predicted hi point: ");
  for (k = 0; k < nDims; ++k)
  ...
  mfa_hilbert_ieee_box_pt(nDims, 1, pointlo, work);
  printf("Predicted lo point: ");
  for (k = 0; k < nDims; ++k)
...
#define nPts 1000000
  assert(mfa_hilbert_ieee_cmp(nDims, pointlo, pointhi) < 0);
  for (i = 0; i < nPts; ++i)
    {
      ...
      for (k = 0; k < nDims; ++k)
  {
    if (i % nDims == k)
      ...
      if (mfa_hilbert_ieee_cmp(nDims, pt1, pt2) < 0)
  {
    if (mfa_hilbert_ieee_cmp(nDims, pt1, pointlo) < 0)
      memcpy(pointlo, pt1, maxDim*sizeof(double));
    if (mfa_hilbert_ieee_cmp(nDims, pointhi, pt2) < 0)
      memcpy(pointhi, pt2, maxDim*sizeof(double));
  }
      else
  {
    if (mfa_hilbert_ieee_cmp(nDims, pt2, pointlo) < 0)
      memcpy(pointlo, pt2, maxDim*sizeof(double));
    if (mfa_hilbert_ieee_cmp(nDims, pointhi, pt1) < 0)
...
int main()
{
  ...
  stat =  mfa_hilbert_nextinbox(nDims, nBytes, 8*nBytes, findPrev, c1, c2, pt);
