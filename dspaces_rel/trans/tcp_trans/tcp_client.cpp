  ...
  closed(false)
{
  ...
  if (connect() ) {
    log_(ERROR, "client_name= " << client_name << " connect failed!")
  }
}
...
{
  if (closed) {
    log_(ERROR, "client client_name= " << client_name << " is already closed!")
    return 1;
  }
  try {
    ...
    // 
    closed = true;
    log_(INFO, "client_name= " << client_name << " closed.")
    return 0;
  }
...
int TCPClient::connect()
{
  try {
    ...
    log_(INFO, "client_name= " << client_name << " connecting to " << endpoint << "...")
...
int TCPClient::send_control_data(std::string control_data, int max_size)
{
  if (control_data.size() > max_size) {
    log_(INFO, "client_name= " << client_name << " control_data.size= " << control_data.size() << " > max_size= " << max_size)
    return 1;
  }
  ...
  if (num_B != max_size) {
    log_(ERROR, "boost::asio::write wrote num_B= " << num_B << " != max_size= " << max_size)
    free(control_data_);
    return 1;
  }
  free(control_data_);
...
{
  send_control_data(boost::lexical_cast<std::string>(data_id), TCP_MAX_DATA_ID_LENGTH);
  send_control_data(boost::lexical_cast<std::string>(data_size), TCP_MAX_DATA_SIZE_LENGTH);
  
  void* _data_ = data_;
  int data_size_to_send = data_size;
  while(data_size_to_send > 0) {
    int chunk_size = (data_size_to_send > TCP_CHUNK_SIZE) ? TCP_CHUNK_SIZE : data_size_to_send;
    int data_size_sent = send_chunk(chunk_size, data_);
    if (data_size_sent == -1) {
      log_(ERROR, "client_name= " << client_name << " send_chunk failed!")
      return 1;
    }
