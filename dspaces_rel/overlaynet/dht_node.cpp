...
: id(id),
  ...
  lip(lip),
  lport(lport),
  joinhost_lip(joinhost_lip),
  joinhost_lport(joinhost_lport),
  join_channel((char*)"join", lip, lport, joinhost_lip, joinhost_lport, boost::bind(&DHTNode::handle_recv, this, _1) ),
  msger(this),
  next_lport(lport)
{
  signal(SIGINT, handle_signal);
  // join_channel.set_recv_callback(boost::bind(&DHTNode::handle_recv, this, _1) );
...
DHTNode::~DHTNode() { LOG(INFO) << "destructed."; }

int DHTNode::get_num_peers() { return ptable.peer_id_vector.size(); }
...
void DHTNode::fill_pptable(std::map<std::string, std::string> msg_map)
{
  ...
  while (1) {
    ...
    char* join_lip = string_to_char_(msg_map["join_lip_" + key_tail_str] );
    int join_lport = boost::lexical_cast<int>(msg_map["join_lport_" + key_tail_str;] );
...
{
  for (std::vector<char>::const_iterator it = ptable.peer_id_vector.begin(); it != ptable.peer_id_vector.end(); it++) {
...
//-----------------------------------------  handle_  --------------------------------------------//
void DHTNode::handle_recv(char* type__srlzed_msg_map)
{
  boost::shared_ptr<Packet> p_ = boost::make_shared<Packet>(strlen(type__srlzed_msg_map), type__srlzed_msg_map);
...
{
  LOG(INFO) << "handle_join_request:: req= \n" << p.to_str();
  ...
  int r = ptable.add_peer(id, id, this->lip, get_next_lport(), lip, lport, join_lip, join_lport);
  if (r == 0) {
    ptable.add_comm_channel(id);
    ptable.set_recv_callback(id, boost::bind(&DHTNode::handle_recv, this, _1) );
  }
  else
