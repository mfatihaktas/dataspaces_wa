...
if [ $1  = 's' ]; then
  ...
  if [ -z "$3" ]; then
    ...
    # GLOG_logtostderr=1 
    export GLOG_logtostderr=1 GLOG_log_dir=$DSPACESWA_DIR
    ./mput_mget_test --type="mput" --cl_id=1 --base_client_id=$(($2*10)) --num_client=$NUM_CLIENT \
                                   --lcontrol_lintf=$LCONTROL_LINTF --lcontrol_lport=${APP_LCONTROL_LPORT_LIST[$3] } --join_lcontrol_lip=${APP_JOIN_LCONTROL_LIP_LIST[$2] } --join_lcontrol_lport=${APP_JOIN_LCONTROL_LPORT_LIST[$2] } \
  ...
  if [ -z "$3" ]; then
    ...
    # GLOG_logtostderr=1 
    export GLOG_logtostderr=1  GLOG_log_dir=$DSPACESWA_DIR
    ./mput_mget_test --type="mget" --cl_id=1 --base_client_id=$(($2*10)) --num_client=$NUM_CLIENT \
                                   --lcontrol_lintf=$LCONTROL_LINTF --lcontrol_lport=${APP_LCONTROL_LPORT_LIST[$3] } --join_lcontrol_lip=${APP_JOIN_LCONTROL_LIP_LIST[$2] } --join_lcontrol_lport=${APP_JOIN_LCONTROL_LPORT_LIST[$2] } \
                                   --num_putget=10 --inter_time_sec=0
  fi
elif [ $1  = 'dp' ]; then
