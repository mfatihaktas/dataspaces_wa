...
std::string DSTest::to_str()
{
  std::stringstream ss;
  ss << "ndim= " << ndim << "\n"
     << "gdim_= " << patch::arr_to_str<>(ndim, gdim_) << "\n"
     << "lb_= " << patch::arr_to_str<>(ndim, lb_) << "\n"
     << "ub_= " << patch::arr_to_str<>(ndim, ub_) << "\n";
...
int DSTest::init(uint64_t data_size)
{
  ...
  for (int i = 0; i < ndim; i++)
    lb_[i] = 0;
  uint64_t ub_limit = ceil(pow(data_size, (float)1/ndim) );
  for (int i = 0; i < ndim; i++) {
    gdim_[i] = ub_limit;
    ub_[i] = ub_limit;
  }
  ...
  if (data_size == 0) {
    ...
    data_ = (char*)malloc(data_size*sizeof(char) );
    
    // int chunk_size = 1000; // *1024*1024;
    // for (int i = 0; i < data_size; i++) {
    //   char c = patch::to_str<>(i/chunk_size).c_str()[0];
    //   data_[i] = c;
    // }
    // for (int i = 0; i < data_size/chunk_size; i++) {
    //   std::string chunk_header = "chunk_" + patch::to_str<>(i);
...
int DSTest::repetitive_put()
{
  ...
  srand(time(NULL) );
  std::string key = arg_struct.base_key + patch::to_str(arg_struct.thread_id);
  while (1) {
    ...
    // log(INFO, "put key= " << key << ", data_= " << patch::arr_to_str<>(data_size, data_) )
    // log(INFO, "put key= " << key << ", writing to file= data_put")
    // return_if_err(patch::write_arr_to_file(data_size, data_, "data_put"), err)
...
int DSTest::repetitive_get()
{
  ...
  while (1) {
    ...
    char* data_to_get_ = (char*)malloc(data_size*sizeof(char) );
    ...
    if (data_size == MSG_SIZE)
      ...
      // log(INFO, "got key= " << key << ", data_= " << patch::arr_to_str<>(data_size, data_) )
      log(INFO, "got key= " << key << ", data_size= " << float(data_size/1024/1024) << " MB.")
      // log(INFO, "got key= " << key << ", writing to file= data_got")
      // return_if_err(patch::write_arr_to_file(data_size, data_to_get_, "data_got"), err)
    }
    free(data_to_get_);
    break;
  }
