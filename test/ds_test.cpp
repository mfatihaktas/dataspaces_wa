...
int DSTest::init(uint64_t data_length)
{
...
// ----------------------------------  Single-threaded  ----------------------------------------- //
int DSTest::exp_put(uint64_t data_length)
{
  int err;
  return_if_err(init(data_length), err)
  std::string key = "thread0";
  
  uint64_t exp_lb_1_[] = {1, 1};
  uint64_t exp_ub_1_[] = {3, 3};
  data_length = DSDriver::get_data_length(ndim, gdim_, exp_lb_1_, exp_ub_1_);
  char* data_to_put_ = (char*)malloc(data_length*sizeof(char) );
  for (int i = 0; i < data_length; i++)
    data_to_put_[i] = static_cast<char>(i%26 + 97);
  return_if_err(ds_driver.sync_put(key.c_str(), ver, sizeof(char), ndim, gdim_, exp_lb_1_, exp_ub_1_, data_to_put_), err)
  log(INFO, "put key= " << key << ", data_length= " << data_length << ", data_= \n" << patch_test::arr_to_str<>(data_length, data_to_put_) )
  free(data_to_put_);
  
  uint64_t exp_lb_2_[] = {1, 4};
  uint64_t exp_ub_2_[] = {3, 5};
  data_length = DSDriver::get_data_length(ndim, gdim_, exp_lb_2_, exp_ub_2_);
  data_to_put_ = (char*)malloc(data_length*sizeof(char) );
  for (int i = 0; i < data_length; i++)
    data_to_put_[i] = static_cast<char>(i%26 + 97);
  return_if_err(ds_driver.sync_put(key.c_str(), ver, sizeof(char), ndim, gdim_, exp_lb_2_, exp_ub_2_, data_to_put_), err)
  log(INFO, "put key= " << key << ", data_length= " << data_length << ", data_= \n" << patch_test::arr_to_str<>(data_length, data_to_put_) )
  free(data_to_put_);
}

int DSTest::exp_get(uint64_t data_length)
{
  int err;
  return_if_err(init(data_length), err)
  std::string key = "thread0";
  ...
  data_length = DSDriver::get_data_length(ndim, gdim_, exp_lb_, exp_ub_);
  char* data_to_get_ = (char*)malloc(data_length*sizeof(char) );
  return_if_err(ds_driver.get(key.c_str(), ver, sizeof(char), ndim, gdim_, exp_lb_, exp_ub_, data_to_get_), err)
  
  log(INFO, "got key= " << key << ", data_length= " << data_length << ", data_= \n" << patch_test::arr_to_str<>(data_length, data_to_get_) )
...
int DSTest::repetitive_put()
{
  ...
  return_if_err(init(arg_struct.data_length), err)
  uint64_t data_length = (arg_struct.data_length == 0) ? MSG_SIZE : arg_struct.data_length;
  ...
  while (1) {
    ...
    return_if_err(ds_driver.sync_put(key.c_str(), ver, sizeof(char), ndim, gdim_, lb_, ub_, data_), err)
    log(INFO, "put key= " << key << ", data_length= " << data_length)
    // log(INFO, "put key= " << key << ", data_length= " << data_length << ", data_= \n" << patch_test::arr_to_str<>(data_length, data_) )
    // log(INFO, "put key= " << key << ", writing to file= data_put")
    // return_if_err(patch_test::write_arr_to_file(data_length, data_, "data_put"), err)
...
int DSTest::repetitive_get()
{
  ...
  while (1) {
    ...
    if (data_length == MSG_SIZE)
      ...
      log(INFO, "got key= " << key << ", data_length= " << float(data_length/1024/1024) << " MB.")
      // log(INFO, "got key= " << key << ", data_length= " << data_length << ", data_= \n" << patch_test::arr_to_str<>(data_length, data_to_get_) )
      // log(INFO, "got key= " << key << ", writing to file= data_got")
      // return_if_err(patch_test::write_arr_to_file(data_length, data_to_get_, "data_got"), err)
    }
    free(data_to_get_);
...
// --------------------------------------------  run_*  ----------------------------------------- //
int DSTest::run_multithreaded_put_test(std::string base_key, uint64_t data_length)
{
  int err;
  for (int i = 0; i < num_putget_threads; i++) {
    ...
    arg_struct.data_length = data_length;
...
int DSTest::run_multithreaded_get_test(std::string base_key, uint64_t data_length)
{
  int err;
  for (int i = 0; i < num_putget_threads; i++) {
    pthread_arg_struct arg_struct;
