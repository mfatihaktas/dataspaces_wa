...
void DDManager::init_ib_server(std::string data_type, const char* lport, data_recv_cb dr_cb)
{
  if (str_equals(data_type, "int") ){
    IBServer<int> ib_server(lport, dr_cb);
    ib_server.init();
  }
  else if (str_equals(data_type, "char") ){
    IBServer<char> ib_server(lport, dr_cb);
    ib_server.init();
  }
  else if (str_equals(data_type, "double") ){
    IBServer<double> ib_server(lport, dr_cb);
    ib_server.init();
  }
  else if (str_equals(data_type, "float") ){
    IBServer<float> ib_server(lport, dr_cb);
    ib_server.init();
  }
}

void DDManager::init_ib_client(std::string data_type, 
                               const char* s_laddr, const char* s_lport, 
                               size_t data_length, void* data_)
{
  if (str_equals(data_type, "int") ){
